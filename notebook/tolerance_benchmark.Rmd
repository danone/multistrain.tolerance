...
all_pipeline %>% group_by(Var2,pipeline) %>% 
  summarise(read_mass=sum(value)) %>% 
  dcast(Var2~pipeline) %>% .[,-1] %>% log10 %>% GGally::ggpairs(upper = list(continuous = "cor", corMethod = "spearman")) + xlab("read mass")



all_pipeline %>% group_by(Var2,pipeline) %>% 
  mutate(prop=(value+1)/sum(value+1)) %>% select(-value)  %>% 
  filter(grepl("Akkermansia", Var1)) %>%
  dcast(Var2~pipeline,value.var="prop") %>% .[,-1] %>% log10 %>% GGally::ggpairs(upper = list(continuous = "cor", corMethod = "spearman")) + xlab("Akkermansia")


all_pipeline %>% group_by(Var2,pipeline) %>% 
  mutate(prop=(value+1)/sum(value+1)) %>% select(-value)  %>% 
  filter(grepl("Bifidobacterium", Var1)) %>%
  dcast(Var2~pipeline,value.var="prop") %>% .[,-1] %>% log10 %>% GGally::ggpairs(upper = list(continuous = "cor", corMethod = "spearman")) + xlab("Bifidobacterium")
...
all_pipeline %>% group_by(Var2,pipeline) %>% 
  ...
  dcast(Var2~pipeline,value.var="prop") %>% .[,-1] %>% log10 %>% GGally::ggpairs(upper = list(continuous = "cor", corMethod = "spearman")) + xlab("Methanobrevibacter")
