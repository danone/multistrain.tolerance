...
design = data.frame(sample=metadata$Subject)
...
genus_usearch_qiime %>% add(1) %>%
  ...
  splsda(., 
         Y = metadata$Visit, 
         ncomp = 2, multilevel = data.frame(sample=metadata$Subject),
keepX = c(10, 10), logratio= 'CLR')


plotIndiv(res.splsda, group = metadata$Visit, ind.names = metadata$Visit,
legend = TRUE)
