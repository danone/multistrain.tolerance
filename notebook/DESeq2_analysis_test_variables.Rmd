...
Tolerance::sample_metadata_dose3v4 %>%
  tibble::column_to_rownames("samplenr") %>% head
  
sample_metadata_dose3v4 %>% head

dds = DESeq2::DESeqDataSetFromMatrix(
  countData = t(genus_count[Tolerance::sample_metadata_dose3v4$samplenr,]), 
  colData = Tolerance::sample_metadata_dose3v4 %>% tibble::column_to_rownames("samplenr"), 
  design = ~ Groupe_dose)
...
temp <- Tolerance::sample_metadata_dose3v4 #%>% tibble::column_to_rownames("samplenr")
temp <- temp[with(temp, order(Groupe_dose, Subject)), ]
new_sub <- data.frame(Reduce(rbind, 
                            by(as.data.frame(temp), 
                                temp$Groupe_dose, 
