...
rbind(  cbind(readxl::read_xlsx("../data-raw/KEGG_annotations/KO_selected_FMP_contribution.xlsx", sheet = "transporter"),
  ...
  mutate(Type=factor(Type, levels=c("Others","MFS transporter", "Aminoacid transporter","Phosphotransferase system","Ion transporter", "Transferase", "Oxidoreductase","Hydrolase","Lyase","Isomerase","Ligase"))) %>%
...
kegg_contrib_V2 %>% #sample_frac(0.1) %>%
  ...
  #KO_contrib_Lactobacillus = read.csv2("KO_contrib_Lactobacillus.csv", row.names=1)
...
## mean contrib by lacto species


```{r}
KO_contrib_Lactobacillus %>%
 merge(kegg_melt, by="KO") %>% dim
...
kegg_melt %>% 
  ...
  merge(tolerance_MSP_table, by="gene_name") %>% 
  filter(gene_name %in% genes_names_from_products) %>%
  merge(KO_contrib_Lactobacillus, by="KO") %>%
  reshape2::dcast(KO~msp_name, value.var = "mean_contrib", fill=0) %>%
  merge(KO_contrib_Lactobacillus, by="KO") %>%
  write.csv2(file="KO_per_lacto_FMP_species.csv")
...
tolerance_spiec_easi_edge_network %>% write.csv2(file="tolerance_spiec_easi_edge_network.csv")
...
graph %>%  
  ...
  with(.,cor.test(log10(.$kegg+1), .$dist_to_lr10, method="spearman"))
...
graph %>%  
  ...
  mutate(kegg=ifelse(is.na(kegg),0,kegg)) %>% 
  select(kegg,dist_to_lr10) %>% as_tibble() %>%
  ggplot() + geom_boxplot(aes(x=dist_to_lr10 %>% as.character, y=log10(kegg+1)))
...
kegg_function_plot = graph %>%  
  ...
  scale_fill_viridis_d("MSP geodesic distance\nto Test product species",option="magma") + 
...
cowplot::plot_grid(kegg_modules_plot, kegg_brite_barplot, network_plot,kegg_function_plot, nrow=2, rel_heights = c(1.2,2), rel_widths = c(1.5,1), labels = "AUTO")
