...
msp_select = unique(msp_select,c("msp_lrhamnosus","msp_1025"))
...
kegg_melt %>% 
  ...
  unique() %>% 
  group_by(msp_name) %>%
  summarise(n=n()) %>% #filter(msp_name=="msp_lrhamnosus")
...
ig.mb     <- adj2igraph(getRefit(MSP_spiec_easi), vertex.attr=list(name=row.names(MSP_abundance_matrix_rounded[msp_select,])))
library(igraph)
...
ggraph(graph) +
  ...
  geom_node_point(aes(color=kegg, size=kegg)) +
