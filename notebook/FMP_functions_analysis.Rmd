...
kegg_contrib_V2 %>% #sample_frac(0.1) %>%
  ...
  merge(kegg_modules, by="KO", all.x = TRUE) %>%
  select(KO,KODesc) %>%
  unique() %>% write_csv2(path="KO_selection_FMP.csv")
...
cowplot::plot_grid(kegg_modules_plot, network_plot,kegg_function_plot, nrow=3, rel_heights = c(1.2,2,1.2), labels = "AUTO")
