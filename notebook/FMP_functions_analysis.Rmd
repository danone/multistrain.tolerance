...
kegg_contrib_V2 %>% #sample_frac(0.1) %>%
  ...
  geom_hline(yintercept = 0.003, linetype=2) +
  scale_color_brewer("", type="qual") +
  xlab("average KO agregated rel. abundance\nat baseline") + 
  ylab("average FMP gene contribution by KO\nafter consumption") +
  theme_minimal()
  
ggsave("KEGG_average_contribution.pdf")
```
...
network_plot = graph %>%  
  ...
  geom_node_point(aes(color=kegg/312,size=11-dist_to_lr10)) +
  #geom_node_text(aes(label=species), size=2) +
  scale_color_viridis("Common KOs\nwith FMP",trans="log10") + 
  scale_size_continuous("Edge distance\nto FMPs", breaks = c(1,4,7,10), labels = c("9","6","3","0")) +
  scale_edge_alpha_continuous("SPIEC-EASI\nweight") + 
  ggforce::geom_mark_ellipse(aes(x, y, label = 'FMP species', 
...
kegg_function_plot = graph %>%  
  ...
  scale_fill_viridis_d("MSP edge distance\nto FMP species",option="magma") + 
  xlab("Number of KOs in common with FMP contributions") +
  ylab("MSP proportion") + theme_minimal()
```


```{r fig.height=10, fig.width=8}
cowplot::plot_grid(network_plot,kegg_function_plot, nrow=2, rel_heights = c(2,1), labels = "AUTO")

ggsave("MSP_network_KEGG_contribution.pdf")
