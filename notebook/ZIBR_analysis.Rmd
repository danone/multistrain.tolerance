...
zibr_est_df %>%
  ...
  ggplot() + 
  geom_point(aes(x=-log10(joint.p.Challenge), y=-log10(joint.p.fx3))) +
  ggrepel::geom_text_repel(aes(x=-log10(joint.p.Challenge), y=-log10(joint.p.fx3), label=G%>%gsub("D_5__","",.))) +
  ylab("Dose effect\n(-log10(Qvalue))") + xlab("Cumulative FMP consumption effect\n(-log10(Qvalue))") + theme_classic() +
  geom_hline(yintercept = -log10(0.05), alpha=0.5, color="pink") + geom_vline(xintercept = -log10(0.05), alpha=0.5, color="pink")
...
sample_metadata %>%
  ...
  ggplot() + geom_point(aes(x=Axis1,y=Axis2,col=Groupe)) + facet_grid(Visit~Doses)

sample_metadata %>%
  merge(genus_clr_pca$li, by.x="samplenr", by.y="row.names") %>%
  filter(Visit %in% c("V3","V4")) %>%
  ggplot() + geom_boxplot(aes(x=Groupe,y=Axis2,fill=Groupe)) + facet_grid(Visit~Doses)
...
genus_clr_pca$co %>%
  ...
  ggrepel::geom_text_repel(aes(x=Comp1,y=Comp2, label=G%>%gsub("D_5__","",.))) +
  theme_classic()
