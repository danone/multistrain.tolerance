...
df.sample_metadata_metaG = data.frame(sample_metadata_metaG[,c("Visit","Groupe_dose", "Groupe", "group","mgs_count")])
...
tiff(file="~/actineo_paper/Rlocal_310719/Microbiome_richness_107samples.tiff", w=800, h=400)
...
tiff(file="~/actineo_paper/Rlocal_310719/Microbiome_richness.tiff", w=800, h=400)
...
# scale_y_continuous(name = 'Delta gene richness', limits=c(0,300)) +
   stat_compare_means(comparisons = my_comparisons_deltagene, label = "p.signif", method="wilcox.test")+
   theme(axis.title.x=element_blank(), legend.position = 'none')+
 # facet_grid(. ~ Visit)+
  theme(strip.text.x = element_text(size=13, color="white", face="bold")) +
  theme(strip.background = element_rect(colour="white", fill="gray48", linetype="solid"))+
...
tiff(file="~/actineo_paper/Rlocal_310719/change_microbiome_richness.tiff", w=800, h=400)
